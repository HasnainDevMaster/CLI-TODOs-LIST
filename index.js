...
const validateInput = (input) => input.trim() !== "" || "Input cannot be empty.";
// Add a new task
const addTask = async () => {
...
const viewTasks = () => {
    if (tasks.length === 0) {
        ...
        tasks.forEach((task) => {
            console.log(`${chalk.yellow(task.id)}. ${task.description} [${task.type}] - ${task.completed ? chalk.green("Completed") : chalk.red("Pending")}`);
        });
    }
};
